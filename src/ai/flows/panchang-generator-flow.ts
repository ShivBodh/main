...
export const PanchangInputSchema = z.object({
  ...
  location: z.string().describe("The location (e.g., 'Ujjain, India') for which to generate the Panchang."),
});
...
export const PanchangOutputSchema = z.object({
  ...
  moonrise: z.string(),
  moonset: z.string(),
  auspiciousTimings: z.array(z.object({ name: z.string(), start: z.string(), end: z.string() })),
  inauspiciousTimings: z.array(z.object({ name: z.string(), start: z.string(), end: z.string() })),
  specialDays: z.array(z.object({ name: z.string(), description: z.string() })),
});
...
export async function getPanchangDetails(input: PanchangInput): Promise<PanchangOutput> {
    ...
    return {
      ...
      inauspiciousTimings: [
        { name: "Rahu Kalam", start: simulatedData.rahuKalam.split(' - ')[0], end: simulatedData.rahuKalam.split(' - ')[1] },
        { name: "Yamaganda", start: simulatedData.yamagandaKalam.split(' - ')[0], end: simulatedData.yamagandaKalam.split(' - ')[1] },
      ],
      specialDays: [
